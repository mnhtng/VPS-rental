...
const PlansPage = () => {
    ...
    const getPlanDescription = (planId: string, fallback?: string): string => {
        try {
            const translated = t(`plan_data.plan-id-${planId}.description`);
    ...
    const getPlanUseCases = (planId: string, fallback: string[]): string[] => {
        try {
            const useCases = t.raw(`plan_data.plan-id-${planId}.use_cases`);
    ...
    const getMemoryFilterLabel = (filter: string) => {
        switch (filter) {
            case 'low': return t('filter.memory_low');
            case 'medium': return t('filter.memory_medium');
            case 'high': return t('filter.memory_high');
                                    ...
                                    <div className="relative flex justify-between gap-6 flex-col lg:flex-row">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                                                    <SelectTrigger className={`h-10 border-gray-300 dark:border-gray-600 transition-all duration-200 hover:border-gray-400 dark:hover:border-gray-500 ${selectedCategory !== 'all' ? 'bg-linear-to-r from-blue-50 to-blue-100 dark:from-blue-950/30 dark:to-blue-900/30 border-blue-400 shadow-sm' : 'bg-white dark:bg-gray-800'}`}>
                                                        <SelectValue placeholder={t('filter.placeholder_category')} />
                                                    </SelectTrigger>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Select value={memoryFilter} onValueChange={setMemoryFilter}>
                                                    <SelectTrigger className={`h-10 border-gray-300 dark:border-gray-600 transition-all duration-200 hover:border-gray-400 dark:hover:border-gray-500 ${memoryFilter !== 'all' ? 'bg-linear-to-r from-orange-50 to-yellow-50 dark:from-orange-950/30 dark:to-yellow-900/30 border-orange-400 shadow-sm' : 'bg-white dark:bg-gray-800'}`}>
                                                        <SelectValue placeholder={t('filter.placeholder_memory')} />
                                                    </SelectTrigger>
                                                    <SelectContent>
                                                        <SelectItem value="all">{t('filter.memory_all')}</SelectItem>
                                                        ...
                                                        <SelectItem value="medium">
                                                            ...
                                                            {t('filter.memory_medium')}
                                                        </SelectItem>
                                                        <SelectItem value="high">
                                                            <div className="flex items-center gap-1.5">
                                                                <div className="w-2 h-2 rounded-full bg-red-500"></div>
                                                            </div>
                                                            {t('filter.memory_high')}
                                                        </SelectItem>
                                                    </SelectContent>
                                                </Select>
                                            </div>
                                        ...
                                        <div className="space-y-3">
                                            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
                                                ...
                                                {t('filter.price_range')}: {formatPrice(priceRange[0])} - {formatPrice(priceRange[1])}
                                            </label>
                            ...
                            >
                                ...
                                <CardHeader className="text-center pb-4">
                                    ...
                                    <CardDescription className="text-muted-foreground">
                                        {getPlanDescription(plan.id, plan.description)}
                                    </CardDescription>
                                    <div className="text-center py-4">
                                        <div className="group-hover:scale-110 transition-transform duration-300">
                                            ...
                                            <span className="text-lg font-normal">{t('card.per_month')}</span>
                                        </div>
                                        <p className="text-sm text-muted-foreground mt-1">{t('card.billed_monthly')}</p>
                                    </div>
                                </CardHeader>
                                ...
                                <CardContent className="flex flex-col justify-between items-center gap-4 space-y-4 w-full h-full">
                                    <div className="grow">
                                        ...
                                        {/* Features List */}
                                        <div className="space-y-2">
                                            {getPlanUseCases(plan.id, plan.use_case).map((useCase, index) => (
                            ...
                            <CardContent className="space-y-6">
                                ...
                                <div className="flex gap-3 justify-center items-center">
                                    ...
                                    >
                                        ...
                                        <span>{t('empty.reset_filters')}</span>
                                    </Button>
                                </div>
                            </CardContent>
                        </Card>
                ...
                {/* Features Comparison */}
                {
                    isLoading ? (
                        ...
                        <Card className="mt-12 hover:shadow-lg transition-shadow animate-in fade-in slide-in-from-bottom duration-700">
                            <CardHeader>
                                <CardTitle className="text-2xl text-center">{t('included.title')}</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    <div className="text-center group hover:scale-105 transition-all duration-300 cursor-pointer">
                                        ...
                                        <h4 className="font-semibold mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{t('included.security')}</h4>
                                        <p className="text-sm text-muted-foreground">{t('included.security_desc')}</p>
                                    </div>
                                    <div className="text-center group hover:scale-105 transition-all duration-300 cursor-pointer">
                                        <Clock className="h-8 w-8 text-green-600 dark:text-green-400 mx-auto mb-3 group-hover:scale-110 transition-transform" />
                                        <h4 className="font-semibold mb-2 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors">{t('included.support')}</h4>
                                        <p className="text-sm text-muted-foreground">{t('included.support_desc')}</p>
                                    </div>
                                    <div className="text-center group hover:scale-105 transition-all duration-300 cursor-pointer">
                                        <Server className="h-8 w-8 text-purple-600 dark:text-purple-400 mx-auto mb-3 group-hover:scale-110 transition-transform" />
                                        <h4 className="font-semibold mb-2 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{t('included.root_access')}</h4>
                                        <p className="text-sm text-muted-foreground">{t('included.root_access_desc')}</p>
                                    </div>
                                    <div className="text-center group hover:scale-105 transition-all duration-300 cursor-pointer">
                                        ...
                                        <h4 className="font-semibold mb-2 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">{t('included.uptime')}</h4>
                                        <p className="text-sm text-muted-foreground">{t('included.uptime_desc')}</p>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
